...
| `yaml`, `pino`, `@octokit/rest` | runtime | Already declared, currently unused | M6 / M2 / M3 make each real.                                                                                                                                                                                                                                                                                                                                                    |

Net new: three devDependencies (`ajv`, `ajv-formats`, `@vitest/coverage-v8`), zero new runtime
dependencies.

### 1.3 HTTP mocking: a hand-rolled `fetch` stub injected through Octokit

**Decision: hand-rolled stub via `new Octokit({ request: { fetch } })`**, not `msw` or `nock`.
...
| ----------------------------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `src/**` except `src/providers/github/**` | `@octokit/*`                                                                                | "No Octokit type escapes `providers/github`" — currently only an `AGENTS.md` sentence |
...
**Decision: read `gh`'s own stored credential file directly, and hand the token to Octokit exactly like
any other resolved token — no subprocess, no CLI dependency, no new capability.** `gh` persists its
...
```ts
// client.ts — the ONLY file that may import @octokit/* (ESLint-enforced)
...
**Exit:** `@octokit/*` resolves from no module outside `src/providers/github/` (verified against the
