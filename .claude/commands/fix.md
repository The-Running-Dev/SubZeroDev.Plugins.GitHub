...
`git status --short` must be clean and on the default branch first, the same guard `/slice` uses (`AGENTS.md`, _Safe start_). Uncommitted work that is not this defect's is not this command's to stash or discard — stop and say so.

## Fix, then hand off

Implement against the issue's agent block. When it is satisfied:

- **Push, then open the pull request. Never as a draft.** Carved out of the authorization rule the same as pushing the branch (`AGENTS.md`, _Git and delivery_).
...
- Fix an adjacent defect noticed along the way. Note it, do not widen the change — the same discipline `resolve.md` and `AGENTS.md`'s _One slice at a time_ already state.
